...
fn main() {
    ...
    fractal_engine.set_sample_path(&String::from("sample/amogus.png")).unwrap();
    ...
    loop {
        ...
        if command == "stop" {
            ...
            fractal_engine.gen_julia(&String::from("random"));
            ...
            let width: u32 = read!();
            let height: u32 = read!();
            println!("=> screen set to {:?}", fractal_engine.set_res(width, height));
        }
            ...
            if func == "make" {
                let name: String = read!();
                fractal_engine.set_seed(&name);
                fractal_engine.gen_image_fractal(&name);
                println!("=> Fractal \"{}\" saved!", name);
                ...
                fractal_engine.set_seed(&seed);
                fractal_engine.gen_image_fractal(&String::from("random"));
                println!("=> Fractal \"{}\" saved!", seed);
            }
            else if func == "set" {
                let pic: String = read!();
                ...
                let hue: u32 = read!();
                println!("=> Sample picture set to {}", fractal_engine.set_image_fractal_hue(hue));
            }
            else {
